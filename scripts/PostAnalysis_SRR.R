...
distanceMat <- DistanceMatrix(cluster_4_sreads, processors=12) #compute distance matrix

clustersId <- IdClusters(distanceMat, method="complete", cutoff=0.01,showPlot = TRUE, myXStringSet = cluster_4_sreads, type = "dendrogram", processors=12)
#clustersId <- IdClusters(method="inexact", cutoff=0.01, myXStringSet = cluster_4_sreads, processors=12)


cluster_4_alignment <- AlignSeqs(cluster_4_sreads, iterations=2, gapOpening=c(-30,-10),gapExtension=c(-5,-1), perfectMatch = 15,misMatch = -3, processors=12)
...
source('scripts/dustyClustering.R')
dustyClustering <- dget
